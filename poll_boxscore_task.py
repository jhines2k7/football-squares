...
def poll_boxscore(game_id, week_id):
  ...
  while game_status != 'closed':
    ...
    num_new_scoring_plays = len(current_scoring_plays) - len(previous_scoring_plays)
    ...
    # logger.info(f"Size new scoring plays: {len(new_scoring_plays)}")
    previous_scoring_plays.clear()
    previous_scoring_plays = current_scoring_plays.copy()
    logger.info(f"Size previous scoring plays: {len(previous_scoring_plays)}")

    # logger.info(f"Size previous scoring plays: {len(previous_scoring_plays)}")
    # scoring_play_dto = ScoringPlayDTO(event_num=count, game_id=game_id, week_id=week_id, scoring_play=scoring_play.model_dump())
    # logger.info(f"Sending event dto: {scoring_play_dto.model_dump_json()}")
    # send_event(scoring_play_dto.model_dump_json())
